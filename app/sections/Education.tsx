...
export default function Education() {
  ...
  return (
    <section
      ref={ref}
      id="education"
      className="relative w-full min-h-screen bg-base-300 py-28 px-4 md:px-8 overflow-hidden text-white scroll-mt-20"
    >
      {/* Background Glow */}
      <motion.div
      ...
      <motion.div
        className="absolute bottom-20 right-10 w-[30rem] h-[30rem] bg-amber-500/10 rounded-full blur-3xl"
        animate={{
          y: [0, 40, 0],
          x: [0, -30, 0],
          opacity: [0.4, 0.7, 0.4],
      ...
      <div className="relative w-full max-w-7xl mx-auto z-10">
        ...
        {educationData.map((edu, index) => {
          ...
          return (
            <motion.div
              key={index}
              className={`relative flex flex-col md:flex-row items-start mb-20 md:mb-24 ${
                isLeft
                  ? "md:justify-start"
                  : "md:justify-end"
              }`}
            ...
            >
              ...
              {/* Education Card */}
              <motion.div
                className={`relative w-full md:w-[44%] p-7 md:p-8 rounded-2xl shadow-xl bg-base-200/70 backdrop-blur-md border border-amber-500/20 hover:border-amber-400/50 transition-all duration-300 ${
                  isLeft
                    ? "md:mr-auto md:pr-10"
                    : "md:ml-auto md:pl-10 md:text-right"
                } mt-16 md:mt-0`}
                whileHover={{
                  scale: 1.03,
                  y: -4,
